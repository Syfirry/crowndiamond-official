...
<link rel="stylesheet" type="text/css" href="<?php echo htmlspecialchars($APP_PATH, ENT_QUOTES, "UTF-8"); ?>/i/css/admin/panel.css" />
...
<script type="text/javascript" src="<?php echo htmlspecialchars($APP_PATH, ENT_QUOTES, "UTF-8"); ?>/i/js/admin/panel.js"></script>
<script type="text/javascript">
function jump(uri){
 // var url = 'index.php?m=admin&' + uri; 
 var url = uri; 
 setCookie('merchant_list_page',1);
 parent.$('#main').attr('src', url);
}
$(document).keydown(function(e) {
    if (e.which == 116 || e.keyCode == 82 && e.ctrlKey) { //116 = F5
        parent.$("#main").attr('src',"<?php echo url(array('m'=>$MOD, 'c'=>'order', 'a'=>'index', ));?>");
        return false;
    }
}); 
</script>
...
<body>
<!-- 头部开始 -->
<?php include $_view_obj->compile('admin/lib/header.html'); ?>
<!-- 头部结束 -->
<div class="container">
  <!-- 菜单开始 -->
...
<!-- 页脚开始 -->
<div class="footer" id="footer"><p>Powered by FlyYaNet Technology Inc. © <?php echo date('Y');?></p></div>
<!-- 页脚结束-->
</body>
</html>