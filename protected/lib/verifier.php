...
class verifier
{
    ...
    /* 数据是否小于等于给定的值
    /*----------------------------------------- */
    public function less_equal_than($val, $right)
    {
        return $val <= $right;
    }
    /*----------------------------------------- */
...
//        return (preg_match('/^$|^1[3|4|5|7|8]\d{9}$/', $val) != 0) == $right;
        return (preg_match('/^\(?([0-9]{3})\)?[-. ]?([0-9]{3})[-. ]?([0-9]{4})$/', $val) != 0 ) == $right;
    }
    /*----------------------------------------- */
    /* 数据是否北美电话格式
    /*----------------------------------------- */
    public static function is_phone_no($val, $right)
    {
        return (preg_match('/^\(?([0-9]{3})\)?[-. ]?([0-9]{3})[-. ]?([0-9]{4})$/', $val) != 0) == $right;
    }
    /*----------------------------------------- */
 ...
 //       return (preg_match('/^$|^[0-9]{6}$/', $val) != 0) == $right;
        return (preg_match('/^[ABCEGHJKLMNPRSTVXY]\d[ABCEGHJKLMNPRSTVWXYZ]( )?\d[ABCEGHJKLMNPRSTVWXYZ]\d$/', $val) != 0) == $right;
    }
    /*----------------------------------------- */
    /* 数据是否是非负整数
    /*----------------------------------------- */
