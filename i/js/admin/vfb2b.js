...
(function($){
  ...
  $.fn.vfLoginFormChecker = function(options){
    ...
    if(form.find('input.loginfielderr').size() == 0){ 
      if(opts.isSubmit){
  ...
  $.fn.dbaFormChecker = function(options){
    ...
    if(form.find('span.dbafielderr').size() == 0){
      if(opts.isSubmit){
        if($.isFunction(opts.beforeSubmit)){
          opts.beforeSubmit.apply(this, arguments);
        }
  ...
  //ajax api no empty $('#rows')
  $.ajaxApi = function(url, dataset, fsuccess){
    $.ajax({
      type: 'post',
      dataType: 'json',
      url: url,
      data: dataset,
      beforeSend: function(){$.vdsLoadingBar(true)},
      success: function(data){$.vdsLoadingBar(false);fsuccess.call($(this), data);},
      error: function(data){
        $.vdsLoadingBar(false);
        $('body').vdsAlert({msg:'处理请求时发生错误'});
      }
