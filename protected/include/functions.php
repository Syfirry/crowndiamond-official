...
{
    $hex = base_convert(md5(microtime().str_shuffle($GLOBALS['config']['encrypt_key'])), 16, $is_numeric ? 10 : 35);
...
function vencrypt($val, $timed = FALSE, $key = null)
{
    ...
    {
        $key = empty($key) ? sha1($GLOBALS['config']['encrypt_key']) : sha1($key);
...
function md5e($string, $salt = 'BAFFINBAY')
{
