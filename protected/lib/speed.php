...
$GLOBALS['config'] = require(APP_DIR.DS.'protected'.DS.'config.php');
require(INCL_DIR.DS.'functions.php');
//require(INCL_DIR.DS.'const.php');
...
if(!empty($GLOBALS['config']['rewrite'])){
	...
	foreach($GLOBALS['config']['rewrite'] as $rule => $mapper){ 
		if('/' == $rule)$rule = '';  
...
//echo $__module.":".$__controller.":".$__action;exit();
			...
			foreach($GLOBALS['config']['rewrite'] as $rule => $mapper){
				...
				if(preg_match($mapper, $route, $matchs)){
					...
					if(0!==stripos($urlArray[$url], 'http://')) 
						$urlArray[$url] = $GLOBALS['config']['protocol_str'].$_SERVER['HTTP_HOST'].rtrim(dirname($_SERVER["SCRIPT_NAME"]), '/\\') .'/'.$urlArray[$url];
...
function url_old($c = 'main', $a = 'index', $param = array()){
    if(is_array($c)){
		$param = $c;
	...
	if($GLOBALS['config']['rewrite'] && ($m == '' || $m == 'mobile' || $m == 'api')){
		static $urlArray=array();
		if(!isset($urlArray[$url])){
			foreach($GLOBALS['config']['rewrite'] as $rule => $mapper){
				$mapper = '/'.str_ireplace(array('/', '<a>', '<c>', '<m>'), 
...
function dump($var, $exit = false){
	...
	if(!$GLOBALS['config']['debug'])return error_log(str_replace("\n", '', $output));
...
class Controller{
	...
	public function display($tpl_name, $return = false){
		if(!$this->_v){
			$compile_dir = isset($GLOBALS['config']['view']['compile_dir']) ? $GLOBALS['config']['view']['compile_dir'] : APP_DIR.DS.'protected'.DS.'tmp';
...
class Model{
	...
	public function create($row){
		...
		return $this->dbInstance($GLOBALS['config']['mysql'], 'master')->lastInsertId();
	}
	...
	public function execute($sql, $params = array(), $readonly = false){
		...
		if($readonly && !empty($GLOBALS['config']['mysql']['MYSQL_SLAVE'])){
			$slave_key = array_rand($GLOBALS['config']['mysql']['MYSQL_SLAVE']);
			$sth = $this->dbInstance($GLOBALS['config']['mysql']['MYSQL_SLAVE'][$slave_key], 'slave_'.$slave_key)->prepare($sql);
		}else{
			$sth = $this->dbInstance($GLOBALS['config']['mysql'], 'master')->prepare($sql);
		}
	...
	public function dbInstance($db_config, $db_config_key, $force_replace = false){
		if($force_replace || empty($GLOBALS['config']['mysql_instances'][$db_config_key])){
			try {
				$GLOBALS['config']['mysql_instances'][$db_config_key] = new PDO('mysql:dbname='.$db_config['MYSQL_DB'].';host='.$db_config['MYSQL_HOST'].';port='.$db_config['MYSQL_PORT'], $db_config['MYSQL_USER'], $db_config['MYSQL_PASS'], array(PDO::MYSQL_ATTR_INIT_COMMAND=>'SET NAMES \''.$db_config['MYSQL_CHARSET'].'\''));
			}catch(PDOException $e){err('Database Err: '.$e->getMessage());}
		}
		return $GLOBALS['config']['mysql_instances'][$db_config_key];
	}
...
function err($msg){
	...
	if(!$GLOBALS['config']['debug']){
		...
		if(!empty($GLOBALS['config']['err_handler'])){
			call_user_func($GLOBALS['config']['err_handler'], $msg, $traces);
		}else{
			error_log($msg);
		}
	}else{
