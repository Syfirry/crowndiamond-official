<?php if(!class_exists("View", false)) exit("no direct access allowed");?><div class="header" id="header">
    <div class="logo fl"><a class="mt5" href="#"><img style="width:90px;height:30px;" src="<?php echo htmlspecialchars($APP_PATH, ENT_QUOTES, "UTF-8"); ?>/i/images/logo_white.png" /></a></div>
    <div class="top-middle fl"><span>天糧書室後台管理系统</span></div>
  <div class="top-links fr cut">
    <span>欢迎您：<?php echo htmlspecialchars($_SESSION['ADMIN']['USERNAME'], ENT_QUOTES, "UTF-8"); ?></span>
    <a title="用户信息" class="icon user" onClick="popAc('pop-user')">用户信息</a>
    <a title="退出登录" class="icon logout" href="<?php echo url(array('m'=>$MOD, 'c'=>'main', 'a'=>'logout', ));?>">退出登錄</a>
  </div>
</div>
<div class="module hdline"></div>
<div class="acpop hide" id="pop-user"> <a class="close" onClick="closeUser()">×</a>
  <h2 class="c999">登錄用戶: <font class="f14 c666 ml5"><?php echo htmlspecialchars($_SESSION['ADMIN']['USERNAME'], ENT_QUOTES, "UTF-8"); ?></font></h2>
  <div class="module poinfo cut">
    ...
    <div class="pwd mt15 hide" id="pwd">
      <form method="post" action="<?php echo url(array('m'=>$MOD, 'c'=>'main', 'a'=>'resetpassword', ));?>">
        <dl>
          <dt>原密码</dt>
