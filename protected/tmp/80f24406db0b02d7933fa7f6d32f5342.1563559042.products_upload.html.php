<?php if(!class_exists("View", false)) exit("no direct access allowed");?><div class="imul impoper cut hide" id="imulpoper">
  <a class="close"></a>
...
<script type="text/javascript" src="<?php echo htmlspecialchars($APP_PATH, ENT_QUOTES, "UTF-8"); ?>/i/webupload/webuploader.min.js"></script>
...
function popUploadImg(){
  ...
  var uploader = WebUploader.create({
    swf: baseUrl + '<?php echo htmlspecialchars($APP_PATH, ENT_QUOTES, "UTF-8"); ?>/i/webupload/Uploader.swf',
    server: "<?php echo url(array('m'=>$MOD, 'c'=>'books', 'a'=>'image', 'step'=>'upload', ));?>",
...
function getGoodsImgList(page_id){
  ...
  $.ajax({
    type: 'post',
    dataType: 'json',
    url: "<?php echo url(array('m'=>$MOD, 'c'=>'products', 'a'=>'image', 'step'=>'list', ));?>",
...
function showGim(src, img_name){
  ...
  $('#gim input[name="products_image"]').val(img_name);
  $('#gimbtns').hide();
}
...
function popUploadAlbum(){
  ...
  var uploader = WebUploader.create({
    swf: baseUrl + '<?php echo htmlspecialchars($APP_PATH, ENT_QUOTES, "UTF-8"); ?>/i/webupload/Uploader.swf',
    server: "<?php echo url(array('m'=>$MOD, 'c'=>'products', 'a'=>'image', 'step'=>'upload', ));?>",
...
<div class="imul impoper cut hide" id="wenanulpoper">
  <a class="close"></a>
  <div class="th"><font class="c888">上传图片</font></div>
  <div class="imuler cut">
    <div class="imulqu"><ul></ul></div>
...
<script type="text/javascript">
function popUploadWenan(){
  $.vdsMasker(true);
  $('#wenanulpoper').vdsMidst({wrapper:$(window), gotop:-100}).show().find('a.close').on('click', function(){
    closeImgUploadPoper();
  });
  ...
  var uploader = WebUploader.create({
    swf: baseUrl + '<?php echo htmlspecialchars($APP_PATH, ENT_QUOTES, "UTF-8"); ?>/i/webupload/Uploader.swf',
    server: "<?php echo url(array('m'=>$MOD, 'c'=>'products', 'a'=>'image', 'step'=>'upload', ));?>",
    formData: {dirtype:'wenan'},
    pick: {
      id: '#wenanulpoper .picker a',
      innerHTML: '点击选择图片'
    },
    dnd: '#wenanulpoper .picker',
    fileSingleSizeLimit:34603008,
 //   compress:false
    resize: false
  });
  ...
  uploader.on('fileQueued', function(file){
    ...
    $li.appendTo('#wenanulpoper .imulqu ul').hover(
      function(){
        $(this).find('.remove').slideDown();
      },function(){
        $(this).find('.remove').slideUp();
      ...
      $('#'+file.id).remove();
      $('#wenanulpoper .picker').show();
      $('#wenanulpoper .bom').hide().off();
      uploader.reset();
    });
    ...
    $('#wenanulpoper .picker a').off().parent().hide();
    $('#wenanulpoper .bom').slideDown(500).find('a').on('click', function(){
      uploader.upload();
    });
  });
  ...
  uploader.on('uploadSuccess', function(file, res){ 
    if(res.error == 'success'){
      $('<p class="hint">上传完毕</p>').appendTo('#'+file.id).fadeIn();
      $('#wenanulpoper .bom a').off().text('确 定').on('click', function(){
        showWenan(res.url, res.name);
        closeImgUploadPoper();
      });
    }else{
      $('<p class="hint">' + res.error + '</p>').appendTo('#'+file.id).fadeIn();
  ...
  var closeImgUploadPoper = function(){
    $.vdsMasker(false);
    $('#wenanulpoper').hide().find('.imulqu ul').empty();
    $('#wenanulpoper .picker').show().next('.morepicker').hide();
    $('#wenanulpoper .bom').hide().find('a').text('开始上传').off();
    uploader.reset();
  }
}
    
function showWenan(src,img_name){
   $('#wenan').show().find('img').attr('src', src).next('i').on('click', function(){
    $('#wenan').hide().find('img').attr('src', '');
    $('#wenanbtns').show();
    $('#wenan input[name="wenan_image"]').val('');
  });
  $('#wenan input[name="wenan_image"]').val(img_name);
  $('#wenanbtns').hide(); 
}
</script>
