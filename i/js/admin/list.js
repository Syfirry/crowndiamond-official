...
function deleteVoucherType(popupid)
{
    var size = $('input[name="id"]:checked').size();
    if(size < 1){ 
      $('body').vdsAlert({msg:"请在列表中选择一项."});
...
function delpurchaseorder(popupid){
   var size = $('input[name="id[]"]:checked').size();
  if(size < 1){ 
    $('body').vdsAlert({msg:"请在列表中选择一项."});
  }else if(size > 1){
